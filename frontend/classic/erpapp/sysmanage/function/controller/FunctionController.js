Ext.define('erp.function.controller.FunctionController', {
	...
	deleteProcess: function(grid, me, record){
		...
		Ext.Msg.confirm('删除系统功能表', tipmessage, function(opts){
			if(opts === 'yes'){
				var options = {source: grid, record: record};
				Delicacy.callOperation('ST_Function', 'delete', data, me.deleteCallback, options);
			}
		});
	},
	...
	deleteCallback: function(resp, options){
		if(resp.ResultSet.status === 0){
			...
			//grid.getStore().removeAt(grid.selectedIndex);
		}
	}
