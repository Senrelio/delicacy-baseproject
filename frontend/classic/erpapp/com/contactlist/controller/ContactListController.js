Ext.define('erp.com.contactlist.controller.ContactListController', {
	...
	onToolAdd: function(){
		...
		var win = Ext.create('erp.com.contactlist.view.ContactListNewWindow', {grid: grid, isNewCreated: true});
		win.loadData();
		win.show();
	},
	...
	onToolSearch: function(){
		var me = this;
		var grid = arguments[0].ownerCt.ownerCt;
		var searchPanel = Delicacy.lookupDockedReference(grid, 'contactlistsearchreference');
	...
	deleteProcess: function(grid, me, data){
		var tipmessage = '确定要删除 '+data.contactName+'?';
		Ext.Msg.confirm('删除ContactList', tipmessage, function(opts){
			if(opts === 'yes'){
				var options = {source: grid};
				Delicacy.callOperation('ST_CrmContact', 'delete', data, me.deleteCallback, options);
			}
		});
	},
	...
	showViewWindow: function(grid, record){
		var win = Ext.create('erp.com.contactlist.view.ContactListViewWindow', {grid: grid});
		win.loadData(record);
		win.show();
	},

	showEditWindow: function(grid, record){
		var win = Ext.create('erp.com.contactlist.view.ContactListEditWindow', {grid: grid});
		win.loadData(record);
		win.show();
	},
