Ext.define('erp.com.contactlist.controller.ContactToCustomerSourceController', {
	extend: 'Ext.app.ViewController',

	alias: 'controller.contactlistcontacttocustomersourceController',

	onToolAdd: function(){
		// 第一个参数是按钮本身，它的上级控件是toolbar，toolbar上级是Grid
		var grid = arguments[0].ownerCt.ownerCt;
		var win = Ext.create('erp.com.contactlist.view.ContactToCustomerSourceNewWindow', {grid: grid, isNewCreated: true});
		win.loadData();
		win.show();
	},
	...
	showViewWindow: function(grid, record){
		var win = Ext.create('erp.com.contactlist.view.ContactToCustomerSourceViewWindow', {grid: grid});
		win.loadData(record);
		win.show();
	},
