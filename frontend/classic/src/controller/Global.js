Ext.define('erp.controller.Global', {
    ...
    beforeHandleRoute: function(id, action) {
		...
		var nav = Ext.StoreMgr.get('navigation');
		//console.log(nav);
		var node = nav.getNodeById(id);
...
*/
    ...
    handleRoute: function(id) {
        ...
        if (node.isLeaf()) {
			...
			if(node.data.executeName)
				executeName = node.data.executeName;
			else
				executeName = '';
            
            (contentPanel.body || contentPanel.el).addCls('kitchensink-example');
			
			Ext.Loader.setPath('erpapp', 'erpapp');
            
			Ext.require(executeName);
			Ext.onReady(function() {
				
				cmp = Ext.create(executeName);
				cmp.isTopLevel = true;
    ...
    onTreeNavSelectionChange: function(selModel, records) {
        ...
        if (record && !record.isRoot()) {
            this.redirectTo(record.getId().toString());
        }
    },

    onBreadcrumbNavSelectionChange: function(breadcrumb, node) {
        if (node) {
            this.redirectTo(node.getId().toString());
        }
    },
