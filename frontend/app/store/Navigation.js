Ext.define('erp.store.Navigation', {
    ...
    load: function (options) {
        ...
        console.log(options);
        if (!options) return;
        if (options && options.node.data && options.node.data.functionId) {
            var params = this.getProxy().getExtraParams();
            params.content = encodeURIComponent(JSON.stringify({
                functionType: 1,
                parentId: options.node.data.functionId
            }));
    ...
    constructor: function (config) {
        ...
        me.callParent([Ext.apply({
            root: this.getNavItemsCalendar()
        }, config)]);

        me.on('rootchange', this.onNavigationRootChange);
        ////debugger;
        var systemMenuData = window.localStorage.getItem(Delicacy._SYSTEM_MENU_DATA);
        if (systemMenuData) {
            var jsonMenuData = JSON.parse(systemMenuData);
            me.setRoot(jsonMenuData);
        }
        // debugger;
        // 即使本地存储已经有了菜单数据，也要从后台重新装入菜单数据
        Delicacy.callOperation('EP_GenerateFunctionTree', 'find', {functionType: 1}, this.callbackProcess, options);
    },

    onNavigationRootChange: function () {
        //debugger;
        if (Delicacy.isNullOrEmpty(arguments[1])) {
            return;
        }
        var newData = arguments[0].treeStore;
        var parentObj = newData.viewref;
        var id = '0';
        var hash = location.hash;
        //console.log(hash);
        if (Delicacy.isNullOrEmpty(hash)) {
            parentObj.redirectTo(id);
            return;
        }
        var idx = hash.lastIndexOf('#');
        if (idx >= 0) {
            id = hash.substring(idx + 1);
        }
        parentObj.redirectTo(id);
    },

    generateNode: function (arr, parentNode) {
        console.log(parentNode);
        for (var i = 0; i < arr.length; i++) {
            var children = new Array();
            if (parentNode.getFunctionId() === arr[i].parentId) {
                children.push(arr[i]);
            }
            for (child in children) {
                var chd = Ext.create('Ext.data.NodeInterface', {
                    text: child.functionName,
                    data: child,
                    id: child.functionId,
                    leaf: child.enabled,
                    functionId: child.functionId
                });
                parentNode.appendChild(chd);
                this.generateNode(arr, chd);
            }
        }
    },
    ...
    callbackProcess: function (resp, options) {
        // debugger;
        var rootNode = Delicacy.generateTreeDataFromResponse(resp);
        //console.log(rootNode);
        var rootNodeString = JSON.stringify(rootNode);
        var originalRootNode = window.localStorage.getItem(Delicacy._SYSTEM_MENU_DATA);
        // if(rootNodeString === originalRootNode) return;
        console.log('start to reset root ...');
        window.localStorage.setItem(Delicacy._SYSTEM_MENU_DATA, JSON.stringify(rootNode));
        //console.log(rootNode);
        options.source.setRoot(rootNode);
        var cmp = Ext.getCmp('navigation-toolbar');
        cmp.updateLayout();
    },
    ...
    getNavItemsGridBasic: function () {
        return {
            ...
            children: [
                {id: 'array-grid', text: '画图', leaf: true},
